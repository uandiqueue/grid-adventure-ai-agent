...
class Agent:
            ...
            for diff_x in range(-1, 2):
                ...
                for diff_y in range(-1, 2):
                    ...
                    for e in new_grid_state.objects_at(check_pos):
                        if isinstance(e, UnlockedDoorEntity) and check_pos in new_locked_doors:
    ...
    def astar_search(self, state: GridState, heuristic_func: Callable, static_map: StaticMap) -> list[Action] | bool:
        ...
        reached = {} # dict[SearchState, int]
        frontier = PriorityQueue()
        initial = state
        initial_search_state = self.parse_search_state(initial)
        reached[initial_search_state] = 0
        curr = Node(parent=None, 
                    ...
                    cost=0)
        num_nodes_expand = 0
        frontier.push((curr.cost + heuristic_func(static_map, initial_search_state)), curr)
        ...
        while frontier.__len__() > 0:
            ...
            if next_node.cost > reached.get(next_search_state, float("inf")):
                continue
            reached[next_search_state] = next_node.cost
            if next_node.state.win:
                ...
                for child_node in self.expand(next_node, next_search_state, static_map):
                    ...
                    if child_search_state in reached:
                        if child_node.cost < reached[child_search_state]:
                            reached[child_search_state] = child_node.cost
                            frontier.push((child_node.cost + heuristic_func(static_map, child_search_state)), child_node)
                        continue
                    elif child_search_state not in reached:
                        reached[child_search_state] = child_node.cost
                        frontier.push((child_node.cost + heuristic_func(static_map, child_search_state)), child_node)
    ...
    def heuristic_func(self, static_map: StaticMap, search_state: SearchState) -> float:
        ...
        movement_turns = self.lower_bound(total_dist, has_speed)
    ...
    def lower_bound(self, distance: int, has_speed: bool = False) -> int:
        if has_speed:
            return (distance + 1) // 2
        return distance
    ...
    def expand(self, node: Node, search_state: SearchState, static_map: StaticMap) -> list[Node]:
        ...
        for action in Action:
            if action == Action.WAIT:
                # Extreme cases like if agent has speed and lava is nearby,
                # and agent will lose if move close to lava
                if search_state.speed_time_left == 0:
                    continue
                if search_state.agent_hp > 3:
                    continue
            elif action == Action.USE_KEY:
